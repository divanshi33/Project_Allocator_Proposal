...
function StudentDashboard() {
  ...
  const fetchAllTopics = useCallback(async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/topics`);
      const grouped = {};
      res.data.forEach((t) => {
  ...
  const fetchFaculties = useCallback(async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/faculty/all`);
  ...
  const fetchPendingRequest = useCallback(async (leaderEmail) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/requests/pending-by-student?studentEmail=${leaderEmail}`
      );
  ...
  const fetchRequestId = useCallback(async (leaderEmail) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/requests/approved?studentEmail=${leaderEmail}`
      );
  ...
  const fetchStudent = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/students/me?email=${email}`
      );
      if (res.data) {
        setStudent(res.data);
  ...
  const sendRequest = async (topic, facultyEmail) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/requests/send`, {
        facultyEmail,
  ...
  const acceptInvitation = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/students/accept-invitation`, {
    ...
    <div className="student-container">
      ...
      <div className="topBar">
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button onClick={logout} style={{ background: "#e53e3e", color: "#fff" }}>Logout</button>
        </div>
      </div>
