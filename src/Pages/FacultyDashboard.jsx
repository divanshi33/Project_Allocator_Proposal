...
function FacultyDashboard() {
  ...
  const fetchStudents = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/students/all`);
  ...
  const fetchTopics = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/topics/${email}`);
      setTopics(res.data);
    } catch (err) { console.log(err); }
  };
  ...
  const fetchProfile = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/faculty/${email}`);
      setFaculty(res.data);
    } catch (err) { console.log(err); }
  };
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/requests/pending?facultyEmail=${email}`
      );
  ...
  const fetchGroupRequests = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/requests/approved-by-faculty?facultyEmail=${email}`
      );
      const map = {};
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_API_BASE_URL}/api/requests/approve/${requestId}`);
  ...
  const handleReject = async (requestId) => {
    setActionLoading(requestId);
    try {
      await axios.patch(`${import.meta.env.VITE_API_BASE_URL}/api/requests/reject/${requestId}`);
      setPendingRequests(prev => prev.filter(r => r._id !== requestId));
    } catch (err) {
      console.log(err);
  ...
  const updateProgress = async (studentEmail, field, value) => {
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/progress/update-progress`, {
  ...
  const deleteTopic = async (id) => {
    await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/topics/delete/${id}`);
    fetchTopics();
  };
  ...
  const updateTopic = async () => {
    await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/topics/update/${editId}`, {
    ...
    <div className="faculty-container">
      ...
      <div className="topBar">
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button className="profileBtn" onClick={logout} style={{ background: "#e53e3e" }}>
            Logout
          </button>
        </div>
      </div>
...
<button
  ...
  onClick={async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/requests/approved?studentEmail=${selectedStudent.email}`
      );
      if (res.data?._id) {
