...
function Signup() {
  ...
  const signupUser = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/otp/send`, { email });
      setStep("otp");
    } catch (err) {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/otp/verify`, { email, otp: otpInput });

      if (role === "student") {
        await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/students/save`, { email, groupDetails: [] });
        navigate("/student");
      } else if (role === "faculty") {
        await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/faculty/create`, { email });
        navigate("/faculty");
      }
    } catch (err) {
