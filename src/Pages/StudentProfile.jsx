...
function StudentProfile() {
  ...
  useEffect(() => {
    ...
    const fetchStudent = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/students/me?email=${email}`);
  ...
  const saveData = async () => {
    ...
    for (let i = 0; i < groupDetails.length; i++) {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/api/students/exists?email=${memberEmail}`
        );
        if (!res.data.exists) {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/students/save`, {
        email,
        groupDetails
      });
      alert("Group Saved!");
